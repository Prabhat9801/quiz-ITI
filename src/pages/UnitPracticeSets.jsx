...
export default function UnitPracticeSets() {
  ...
  return (
    <div>
      ...
      {/* Step 1: Unit list — only shown until a unit is picked */}
      {!openUnit && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
          {index.units.map((u) => (
            <button
              ...
              className="rounded-lg border border-slate-200 bg-white p-2.5 sm:p-3 text-left text-slate-700 transition hover:border-indigo-300"
            >
      ...
      {/* Step 2: Sets for the chosen unit — only shown until a set is picked */}
      {openUnit && !openSetInfo && (
        <div>
          <button
            type="button"
            onClick={backToUnits}
            className="mb-3 text-sm font-medium text-indigo-700 hover:underline"
          >
            ← Unit badlo
          </button>
          <h2 className="font-semibold text-slate-900 mb-2">{openUnit.unitName}</h2>
          <div className="space-y-2">
            {openUnit.sets.map((s) => (
              <button
                key={s.setNumber}
                type="button"
                onClick={() => setOpenSet(s.setNumber)}
                className="w-full rounded-lg border border-slate-200 bg-white p-3 text-left transition hover:border-indigo-300"
              >
      ...
      {/* Step 3: Chosen set's timer/start/PDF panel */}
      {openSetInfo && (
